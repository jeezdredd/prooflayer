...
class TestValidatePublicUrl:
    ...
    @patch("common.url_safety._dns_lookup", return_value=["10.0.0.5"])
    ...
    @patch("common.url_safety._dns_lookup", return_value=["224.0.0.1"])
    def test_multicast_blocked(self, mock_dns):
        with pytest.raises(UnsafeUrlError):
            validate_public_url("http://mc/x")

    @patch("common.url_safety._dns_lookup", return_value=["240.0.0.1"])
    def test_reserved_blocked(self, mock_dns):
        with pytest.raises(UnsafeUrlError):
            validate_public_url("http://res/x")

    @patch("common.url_safety._dns_lookup", return_value=["::1"])
...
class TestSafeGet:
    ...
    @patch("common.url_safety._dns_lookup")
