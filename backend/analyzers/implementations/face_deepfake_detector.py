    ...
    if _state["model"] is None:
        try:
            model = AutoModelForImageClassification.from_pretrained(
                MODEL_NAME, local_files_only=False, use_safetensors=True
            ).eval()
        except Exception:
            model = AutoModelForImageClassification.from_pretrained(
        ...
        logger.info("face_deepfake_detector loaded %s on %s", MODEL_NAME, get_device())
...
class FaceDeepfakeDetector(BaseAnalyzer):
    ...
    def analyze(self, file_path: str, metadata: dict) -> AnalysisOutput:
        ...
        try:
            ...
            logger.warning("face_deepfake_detector inference failed: %s", exc)
