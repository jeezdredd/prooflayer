...
from analyzers.implementations.face_deepfake_detector import FaceDeepfakeDetector, _fake_index
...
class TestFaceDeepfakeDetector:
    ...
    def test_high_fake_prob_returns_fake(self, analyzer, jpeg_path):
        with patch("analyzers.implementations.face_deepfake_detector._load", return_value=_fake_load_logits(0.0, 4.0)):
    ...
    def test_low_fake_prob_returns_authentic(self, analyzer, jpeg_path):
        with patch("analyzers.implementations.face_deepfake_detector._load", return_value=_fake_load_logits(4.0, 0.0)):
            output = analyzer.analyze(jpeg_path, {})
        assert output.verdict == "authentic"

    def test_mid_prob_returns_inconclusive(self, analyzer, jpeg_path):
        with patch("analyzers.implementations.face_deepfake_detector._load", return_value=_fake_load_logits(0.0, 0.0)):
            output = analyzer.analyze(jpeg_path, {})
        assert output.verdict == "inconclusive"

    def test_suspicious_range(self, analyzer, jpeg_path):
        with patch("analyzers.implementations.face_deepfake_detector._load", return_value=_fake_load_logits(0.0, 1.5)):
            output = analyzer.analyze(jpeg_path, {})
        assert output.verdict == "suspicious"

    def test_evidence_contains_ai_probability(self, analyzer, jpeg_path):
        with patch("analyzers.implementations.face_deepfake_detector._load", return_value=_fake_load_logits(1.0, 0.0)):
            output = analyzer.analyze(jpeg_path, {})
    ...
    def test_inference_error_returns_error_verdict(self, analyzer, jpeg_path):
        with patch("analyzers.implementations.face_deepfake_detector._load", side_effect=RuntimeError("model error")):
            output = analyzer.analyze(jpeg_path, {})
        assert output.verdict == "error"
