...
from content.views import ANONYMOUS_OWNER_EMAIL
...
class AnalyzeUrlViewTest(TestCase):
    ...
    @patch("common.url_safety._dns_lookup", return_value=["127.0.0.1"])
    ...
    @patch("common.url_safety._dns_lookup", return_value=["169.254.169.254"])
    def test_ssrf_link_local(self, mock_dns):
        response = self.client.post(self.URL, {"url": "http://metadata.internal/img.jpg"}, format="json")
        assert response.status_code == 400
        assert response.data["detail"] == "URL not allowed"

    @patch("common.url_safety._dns_lookup", return_value=["93.184.216.34"])
    ...
    @patch("content.views.process_submission.delay")
    @patch("content.views.safe_get")
    def test_anonymous_submission_owned_by_service_account(
        self, mock_get, mock_delay, mock_mime, mock_quota, mock_dns
    ):
        mock_resp = MagicMock()
        mock_resp.raise_for_status.return_value = None
        mock_resp.iter_content.return_value = [b"\xff\xd8\xff" + b"x" * 100]
        ...
        assert response.status_code == 201
        owner = Submission.objects.get(id=response.data["submission_id"]).user
        assert owner.email == ANONYMOUS_OWNER_EMAIL
    ...
    @patch("content.views.process_submission.delay")
    @patch("content.views.safe_get")
    def test_repeated_anonymous_calls_reuse_one_service_account(
        self, mock_get, mock_delay, mock_mime, mock_quota, mock_dns
    ):
        mock_resp = MagicMock()
        mock_resp.raise_for_status.return_value = None
        ...
        self.client.post(self.URL, {"url": "http://example.com/a.jpg"}, format="json")
        self.client.post(self.URL, {"url": "http://example.com/b.jpg"}, format="json")

        assert User.objects.filter(email=ANONYMOUS_OWNER_EMAIL).count() == 1
